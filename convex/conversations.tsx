...
export const get = query({
    args: {}, handler: async (ctx, args) => {
        ...
        const conversationMemberships = await ctx.db.query
        ("conversationMembers").withIndex
        ("by_memberId", q => q.eq("memberId", currentUser._id))
        .collect()

        const conversations = await Promise.all(conversationMemberships?.map
            (async  membership=> {
                const conversation = await ctx.
                db.get(membership.conversationId);

                if(!conversation) {
                    throw new ConvexError("convo could not be found");
                }
            ...
            await Promise.all(conversations.map
                (async (conversation, index) => {
                    ...
                    await ctx.db.query("conversationMembers")
