...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if(
            friends1.some((friend)=> 
            friend.user2 === receiver._id) ||
            friends2.some((friend)=> 
                friend.user1 === receiver._id)
         ) {
            throw new ConvexError("You are already friends with");
         }
...
export const accept = mutation({
    args: {
        ...
        if(!request || request.receiver !== currentUser._id) {
            throw new ConvexError("ADA ERROR BRO WAKTU REQUEST");
        }
        ...
        await ctx.db.insert("conversationMembers", {
            memberId: currentUser._id, conversationId
        })
        await ctx.db.insert("conversationMembers", {
            memberId: currentUser._id, conversationId
        })
