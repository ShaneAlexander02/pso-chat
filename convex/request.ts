...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (requestAlreadySent) {
            throw new ConvexError("Request alreadys sent");
        }
        ...
        ("user1", currentUser._id)).collect();

        const friends2 = await ctx.db
        .query("friends")
        .withIndex("by_user2", (q) => q.eq
        ("user2", currentUser._id)).collect();
        ...
        if(
            friends1.some((friend) => friend
            .user2 === receiver._id) ||
            friends2.some((friend)=> friend 
            .user1 === receiver._id)
        ){
            throw new ConvexError("You are already friendsss with this user");
        }
...
export const deny = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if(!currentUser) {
            throw new ConvexError("User not foundsss")
        }
...
export const accept = mutation({
    args: {
        ...
        if(!request || request.receiver !== currentUser._id) {
            throw new ConvexError("There was an error accpeting thisss request");
        }
